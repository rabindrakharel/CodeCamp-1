...
var nepaviz = (function() {
    ...
    return {
            ...
            nepal(function() {
                ...
                data(function() {
                    ...
                    g.selectAll("." + type)
                        .data(layer.features)
                        .enter().append("path")
                        .attr("class", function(d) { return type + " " + d.id + " q" + quantile(dataById[d.id]) + "-" + range; })
                        .attr("d", path)
                        .on("mouseover", function(d) {
							...
							div.html(function(a) {
                                ...
                                info = info.slice(0, -2);
                                info += "</p><hr/><table>";
                                keys.forEach(function(a, i) { info += "<tr><td>" + ((a === field) ? "<b>" + a + "</b>" : a)  + "</td><td>" + ((a === field) ? "<b>" + values[i] + "</b>" : values[i]) + "</td></tr>"; });
                                info += "</table>";
                        ...
                        .on("click", function(d) {
                            if (d3.event.altKey) {
                                ...
                                var x = d3.scale.linear().domain([0, d3.max(d3.values(unitData))]).range([0, 420]);
                                var chart = d3.select("#chart").append("svg")
                                    .attr("width", 500)
                                    .attr("height", 500);

                                chart.selectAll("rect")
                                        .data(d3.values(unitData))
                                    .enter().append("rect")
            ...
            nepal(function() {
                ...
                data(function() {
                    ...
                    g.selectAll("." + type)
                        ...
                        .enter().append("path")
                        .attr("class", function(d) { return childType + " " + d.id + " q" + quantile(dataById[d.id]) + "-" + range; })
                        .attr("d", path)
                        .on("mouseover", function(d) {
                            d3.select(this)
                                .classed("active", true);
        ...
        //plotLatLong
        plotLatLong: function(level, latLong) {
            var layer,
                type,
                bounds,
                scale,
